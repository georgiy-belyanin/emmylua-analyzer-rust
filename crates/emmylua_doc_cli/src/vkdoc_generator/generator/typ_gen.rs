...
fn generate_class_type_markdown(
    ...
    let typ_name = typ.get_full_name();
    ...
    if let Some(members) = members {
        for member in members {
            ...
            if member_typ.is_function() {
                ...
                let typ_display = humanize_type(db, member_typ, RenderLevel::Documentation);
                field_members.push(MemberDoc {
                    name: title_name,
    ...
    //    &Path::new("types").join(
    //        escape_type_name(typ.get_full_name())
    //            .replace(".", "-")
    //            .as_str(),
    //    ),
    //    vkdoc::VkdocEntry {
    ...
    //);
    mkdocs_index.insert_vkdoc(
        /*typ.get_namespace()
        .map(|v| v.to_string())
        .unwrap_or("".to_string())*/
        typ.get_full_name().to_string(),
        crate::vkdoc_generator::markdown_types::VkdocKind::Type,
        render_text,
    );
    Some(())
}
...
fn generate_alias_type_markdown(
    ...
    if let Some(origin_typ) = typ.get_alias_origin(db, None) {
        let origin_type_display = humanize_type(db, &origin_typ, RenderLevel::Documentation);
        let display = format!(
            "```lua\n(alias) {} = {}\n```\n",
