...
pub fn generate_module_markdown(
    ...
    //    &Path::new("modules").join(
    //        escape_type_name(&module.full_module_name)
    //            .replace(".", "-")
    //            .as_str(),
    //    ),
    //    vkdoc::VkdocEntry {
    ...
    mkdocs_index.insert_vkdoc(
        module.full_module_name.clone(),
        crate::vkdoc_generator::markdown_types::VkdocKind::Module,
        render_text,
    );
    Some(())
}
            ...
            if member_type.is_function() {
                ...
                match member_type {
                    LuaType::TableConst(table) => {
                        let mut context = tera::Context::new();
                        let template_name = "lua_global_template.tl";
                        let member_owner = LuaMemberOwner::Element(table.clone());
                        let members = db.get_member_index().get_sorted_members(&member_owner);

                        let mut doc = Doc::default();
                        doc.name = title_name.clone();
                        doc.property = member_property;
                        generate_member_owner_module(
                            db,
                            member_owner,
                            &title_name,
