...
pub fn generate_global_markdown(
    ...
    match decl_type.as_type() {
        LuaType::TableConst(table) => {
            ...
            generate_member_owner_module(db, member_owner, name, &mut doc, tl, mkdocs_index)?;
        }
        _ => {
    ...
    mkdocs_index.insert_vkdoc(
        ...
        crate::vkdoc_generator::markdown_types::VkdocKind::Global,
        render_text,
    );
    Some(())
}
...
fn generate_simple_global(db: &DbIndex, decl: &LuaDecl, doc: &mut Doc) -> Option<()> {
    ...
    if ty.is_function() {
        ...
        let typ_display = humanize_type(db, ty, RenderLevel::Documentation);
        let display = format!("```lua\n{} : {}\n```\n", name, typ_display);
        doc.display = Some(display);
    }
